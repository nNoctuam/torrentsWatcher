...
func (t *Transmission) AddTorrent(content []byte, dir string, paused bool) (torrent driven.Torrent, err error) {
	var responseModel struct {
		Arguments struct {
			TorrentAdded     driven.Torrent `json:"torrent-added"`
			TorrentDuplicate driven.Torrent `json:"torrent-duplicate"`
		} `json:"arguments"`
	...
	if err != nil {
		return driven.Torrent{}, err
	}

	if responseModel.Result != successResult {
		return driven.Torrent{}, errors.New("torrent-add result: " + responseModel.Result)
	}
...
func (t *Transmission) GetTorrents() ([]driven.Torrent, error) {
	var responseModel struct {
		Arguments struct {
			Torrents []driven.Torrent `json:"torrents"`
		} `json:"arguments"`
	}
...
func (t *Transmission) GetTorrentFiles(ids []int) ([]driven.TorrentFile, error) {
	var responseModel struct {
		Arguments struct {
			Torrents []driven.Torrent `json:"torrents"`
		} `json:"arguments"`
